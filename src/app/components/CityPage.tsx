...
import { HomeFacade, ProjectsFacade, ExperienceFacade, SkillsFacade } from './PixelStreetFacades';
import { PortfolioPage, type PortfolioSection } from './PortfolioPage';
...
interface LocationState {
  character?: { style: string; hair: string; hairColor: string; gender: string; skin: string; top?: string; }
}

// ─── Constants ───────────────────────────────────────────────────────────────
const LOCATIONS = [
  { id: 'home',       section: 'home'       as PortfolioSection, name: 'HOME',       x: 180,  width: 280, height: 480, Component: HomeFacade },
  { id: 'projects',   section: 'projects'   as PortfolioSection, name: 'PROJECTS',   x: 620,  width: 300, height: 520, Component: ProjectsFacade },
  { id: 'experience', section: 'experience' as PortfolioSection, name: 'EXPERIENCE', x: 1100, width: 290, height: 540, Component: ExperienceFacade },
  { id: 'skills',     section: 'skills'     as PortfolioSection, name: 'SKILLS',     x: 1560, width: 270, height: 490, Component: SkillsFacade },
];
...
export function CityPage() {
  ...
  const characterData = state?.character || getSavedCharacter() || {
    style: 'STREETWEAR', hair: 'SPIKY', hairColor: '#4a3020', gender: 'NEUTRAL', skin: '#ffdbac', top: '',
  };
  ...
  type SectionState = 'closed' | 'fadeIn' | 'open' | 'fadeOut';
  const [sectionState, setSectionState] = useState<SectionState>('closed');
  const [activeSection, setActiveSection] = useState<PortfolioSection>('home');
  const sectionTimers = useRef<number[]>([]);
  const afterSec = (fn: () => void, ms: number) => {
    const id = window.setTimeout(fn, ms);
    sectionTimers.current.push(id);
  };
  useEffect(() => () => { sectionTimers.current.forEach(clearTimeout); }, []);
  ...
  const handleEnterLocation = (locId: string) => {
    ...
    if (loc && sectionState === 'closed') {
      setActiveSection(loc.section);
      setSectionState('fadeIn');
      afterSec(() => setSectionState('open'), 400);
    } else {
  ...
  const handleSectionExit = () => {
    setSectionState('fadeOut');
    afterSec(() => { setSectionState('closed'); keysPressed.current.clear(); storeOpenRef.current = false; }, 400);
  };
  ...
  return (
    <div className="relative w-full h-screen bg-[#050505] overflow-hidden font-sans select-none">
      ...
      {/* Section overlay */}
      <PortfolioPage section={activeSection} state={sectionState} onExit={handleSectionExit} />
    </div>
  );
}