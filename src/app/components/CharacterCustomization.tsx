...
const CATEGORIES = ['style', 'gender', 'topColor', 'hair', 'hairColor', 'skin'] as const;
type Category = typeof CATEGORIES[number];

// ─── Job "classes" — each maps to one underlying pixel outfit so the character
// still renders correctly. `outfit` is the value stored in state / localStorage.
const ROLES = [
  { label: 'SOFTWARE ENGINEER', outfit: 'STREETWEAR', color: '#00ffff' },
  { label: 'AI ENGINEER',       outfit: 'TECHWEAR',   color: '#ff66cc' },
  { label: 'PRODUCT MANAGER',   outfit: 'VINTAGE',    color: '#ffaa00' },
  { label: 'DESIGNER',          outfit: 'SCHOOL',     color: '#44ff88' },
];

// Quick lookup: outfit key → class color
const OUTFIT_COLOR: Record<string, string> = Object.fromEntries(ROLES.map(r => [r.outfit, r.color]));

// Fun persona shown under the character, keyed by outfit + gender expression.
const PERSONAS: Record<string, Record<string, string>> = {
  STREETWEAR: { FEM: 'FRONTEND NINJA',    MASC: 'BACKEND HACKER',  NEUTRAL: 'FULL-STACK DEV' },
  TECHWEAR:   { FEM: 'NEURAL ARCHITECT',  MASC: 'MODEL WRANGLER',  NEUTRAL: 'PROMPT ALCHEMIST' },
  VINTAGE:    { FEM: 'ROADMAP STRATEGIST',MASC: 'SHIP CAPTAIN',    NEUTRAL: 'PRODUCT VISIONARY' },
  SCHOOL:     { FEM: 'PIXEL ARTIST',      MASC: 'UX RONIN',        NEUTRAL: 'DESIGN SYSTEMS LEAD' },
};
...
export function CharacterCustomization() {
  ...
  // Outfit "paint" — '' keeps the class default; the rest override the top color
  const topColors = [
    { name: 'Auto',   value: '' },
    { name: 'Black',  value: '#1a1a1a' },
    { name: 'White',  value: '#e8e8e8' },
    { name: 'Red',    value: '#cc2233' },
    { name: 'Orange', value: '#ff8800' },
    { name: 'Yellow', value: '#ffcc33' },
    { name: 'Green',  value: '#33aa55' },
    { name: 'Teal',   value: '#22bb99' },
    { name: 'Blue',   value: '#3366dd' },
    { name: 'Purple', value: '#9944cc' },
    { name: 'Pink',   value: '#ff66aa' },
  ];

  // Use a ref so the keyboard handler always reads fresh state without re-registering
  const stateRef = useRef({ style, hair, hairColor, gender, skin, topColor, focusCatIdx, styles, hairs, skins, genders, hairColors, topColors });
  useEffect(() => { stateRef.current = { style, hair, hairColor, gender, skin, topColor, focusCatIdx, styles, hairs, skins, genders, hairColors, topColors }; });
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      switch (cat) {
        ...
        case 'topColor':  options = s.topColors.map(c => c.value); currentVal = s.topColor;  setter = setTopColor;  break;
      ...
      if (e.key === 'ArrowUp' || e.key === 'w' || e.key === 'W') {
        ...
        const { style: st, hair: hr, hairColor: hc, gender: gn, skin: sk, topColor: tc } = stateRef.current;
        setIsExiting(true);
        localStorage.setItem('to_character', JSON.stringify({ style: st, hair: hr, hairColor: hc, gender: gn, skin: sk, top: tc }));
        setTimeout(() => {
          navigate('/city', { state: { character: { style: st, hair: hr, hairColor: hc, gender: gn, skin: sk, top: tc } } });
        }, 600);
      }
    };
  ...
  const handleEnterCity = () => {
    ...
    localStorage.setItem('to_character', JSON.stringify({ style, hair, hairColor, gender, skin, top: topColor }));

    // Wait for fade out animation
    setTimeout(() => {
        navigate('/city', {
            state: {
                character: { style, hair, hairColor, gender, skin, top: topColor }
            }
        });
    }, 600);
  };
      ...
      <div className="relative z-20 w-full h-full flex flex-col items-center py-6 md:py-12 overflow-y-auto">
        ...
        <motion.button
          ...
          style={{
            ...
            boxShadow: '3px 3px 0 #000, 0 0 10px #00ffff44',
            letterSpacing: '0.08em',
            cursor: 'pointer',
          }}
          whileHover={{ scale: 1.05, boxShadow: '3px 3px 0 #000, 0 0 16px #00ffff88' }}
          whileTap={{ scale: 0.95 }}
        >
          ◀ HOME
        </motion.button>
        ...
        >
          ...
          <div className="mt-4 flex items-center justify-center gap-2 flex-wrap px-4">
            {CATEGORIES.map((cat, i) => {
              const labels: Record<Category, string> = { style: 'CLASS', gender: 'GENDER', topColor: 'PAINT', hair: 'HAIR', hairColor: 'COLOR', skin: 'SKIN' };
              const colors: Record<Category, string> = { style: '#ff00ff', gender: '#ff66aa', topColor: '#ff8855', hair: '#00ffff', hairColor: '#44ffdd', skin: '#ffaa44' };
        ...
        <div className="flex flex-col md:flex-row w-full max-w-6xl items-center justify-center gap-5 md:gap-16 px-4 flex-1">
            ...
            {/* Left Panel: STYLE */}
            <Panel title="CLASS" side="left" highlight={focusCat === 'style' || focusCat === 'gender' || focusCat === 'topColor'}>
               <div className={`flex flex-col gap-3 rounded transition-all duration-150 ${focusCat === 'style' ? 'ring-1 ring-[#ff00ff]/40 p-1' : 'p-1'}`}>
                 {ROLES.map((r) => (
                    <OptionButton
                       key={r.outfit}
                       label={r.label}
                       selected={style === r.outfit}
                       onClick={() => setStyle(r.outfit)}
                       color={r.color}
                    />
                 ))}
               </div>
               ...
               {/* Outfit Paint (top color) */}
               <div className={`mt-6 border-t-2 border-white/10 pt-4 rounded transition-all duration-150 ${focusCat === 'topColor' ? 'ring-1 ring-[#ff8855]/40 p-1' : 'p-1'}`}>
                 <p className="text-[10px] mb-3 opacity-80" style={{ fontFamily: '"Press Start 2P"', color: '#ff8855' }}>OUTFIT PAINT</p>
                 <div className="grid grid-cols-4 gap-2">
                    {topColors.map((c) => (
                        <button
                          key={c.name}
                          onClick={() => setTopColor(c.value)}
                          className={`w-full aspect-square border-2 flex items-center justify-center transition-all duration-200 ${topColor === c.value ? 'border-white scale-110 shadow-[0_0_8px_white]' : 'border-gray-600 hover:scale-105'}`}
                          style={{ backgroundColor: c.value || 'transparent' }}
                          title={c.name}
                        >
                          {c.value === '' && <span className="text-[7px] text-white/70" style={{ fontFamily: '"Press Start 2P"' }}>A</span>}
                        </button>
                    ))}
                 </div>
               </div>
            ...
            <div className="relative flex flex-col items-center justify-center order-first md:order-none mb-8 md:mb-0">
               <div className="relative w-36 h-48 md:w-64 md:h-80 flex items-center justify-center">
                  ...
                  <div className="relative z-10 drop-shadow-[0_10px_20px_rgba(0,0,0,0.8)] filter drop-shadow-[0_0_15px_rgba(255,255,255,0.1)] transition-transform duration-300 hover:scale-105">
                     <PixelCharacter
                        ...
                        skin={skin}
                        scale={8}
                        accent={OUTFIT_COLOR[style]}
                        top={topColor || undefined}
                     />
                  </div>
               </div>
               ...
               >
                 ...
                 <div className="text-sm md:text-base" style={{ fontFamily: '"Press Start 2P"', color: OUTFIT_COLOR[style], textShadow: `0 0 10px ${OUTFIT_COLOR[style]}88` }}>
                   {PERSONAS[style]?.[gender] ?? ''}
                 </div>
               </motion.div>
            </div>
...
function OptionButton({ label, selected, onClick, color }: { label: string, selected: boolean, onClick: () => void, color: string }) {
    return (
        <button 
           onClick={onClick}
           className={`w-full py-4 px-4 text-[9px] md:text-[11px] leading-relaxed text-left transition-all duration-200 border-l-4 flex justify-between items-center group relative overflow-hidden ${selected ? 'bg-white/10 pl-6' : 'hover:bg-white/5 hover:pl-5'}`}
           style={{ 
               fontFamily: '"Press Start 2P"',
